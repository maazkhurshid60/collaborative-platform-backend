...
const getGroupMessageApi = (0, asyncHandler_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const groupMessagesWithReadStatus = groupMessages.map(message => (Object.assign(Object.assign({}, message), { readStatus: message.groupReadReceipts.length > 0 ? 'read' : 'unread' })));
    return res.status(http_status_codes_1.StatusCodes.OK).json(new apiResponse_1.ApiResponse(http_status_codes_1.StatusCodes.OK, { groupMessages: groupMessagesWithReadStatus }, 'Fetched group messages successfully'));
}));
...
//     }
//     console.log("await prisma.chatMessage.findMany", await prisma.chatMessage.findMany());
//     // Fetch all messages that belong to the specific group, along with sender information
//     const groupMessages = await prisma.chatMessage.findMany({
//         where: {
//             groupId: groupId,  // Ensure that the message belongs to the specific group
//         },
//         include: {
//             sender: {
//                 include: {
//                     user: true, // Include sender's user data
//                 },
//             },
//         },
//     });
//     if (!groupMessages || groupMessages.length === 0) {
//         return res.status(StatusCodes.NOT_FOUND).json(new ApiResponse(StatusCodes.NOT_FOUND, { message: "No messages found for this group." }, "Messages Not Found"));
//     }
//     return res.status(StatusCodes.OK).json(
//         new ApiResponse(StatusCodes.OK, { groupMessages }, 'Fetched group messages successfully')
//     );
// });
...
//         data: {
//             sender: { connect: { id: senderId } }, // Connect the sender using senderId
//             message,
//             mediaUrl: mediaUrl || '',
//             type: type || 'text',
//             group: {
