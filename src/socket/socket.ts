...
async function authorizeCallJoin(
  appointmentId: string,
  token: string | undefined,
): Promise<CallAuthResult | null> {
  ...
  if (payload.role === "provider") {
    ...
    const isBookingProvider =
      appointment.bookingProvider &&
      payload.participantId === appointment.bookingProvider.userId;
...
async function logCallEvent(
  ...
  try {
    ...
    logger.error(
      `Failed to write call log (${event}) for appointment ${appointmentId}: ${err.message}`,
    );
  }
}
...
const activeCallRooms = new Map<
  string,
  { appointmentId: string; endTime: Date }
>();
    ...
    for (const [room, info] of activeCallRooms) {
      ...
      try {
        ...
        for (const s of sockets) {
          const participant = (s.data as { callParticipant?: CallAuthResult })
            .callParticipant;
          if (participant) {
            await logCallEvent(
              info.appointmentId,
        ...
        logger.error(
          `Call expiry sweep failed for room ${room}: ${err.message}`,
        );
      }
...
export function setupSocket(server: any) {
  ...
  logger.info(
    "Redis apdapter is ready, socket.io is now spreading in multi servers",
  );
  ...
  io.on("connection", async (socket: any) => {
    ...
    const activeId = (userId || providerId)?.toString();

    if (activeId) {
      userStatusMap.set(activeId, {
        isOnline: true,
    ...
    logger.info(
      `Connected: sockeetId=${socket.id}, userId=${userId}, providerId=${providerId}`,
    );
    ...
    socket.on(
      ...
      ({ targetUserId }: { targetUserId: string }) => {
        ...
        const status = userStatusMap.get(targetUserId.toString()) || {
          isOnline: false,
          lastSeen: new Date().toISOString(),
        };
        socket.emit("user_status_response", { targetUserId, ...status });
      },
    );
    ...
    socket.on(
      "join_channel",
      ({ chatChannelId }: { chatChannelId: string }) => {
    ...
    socket.on(
      ...
      async ({
        toProviderId,
        message,
      }: { toProviderId?: string; message?: any } = {}) => {
        try {
          const msg = message || {};
          const channelId = msg.chatChannelId || msg.channelId;

          if (channelId) {
            socket.join(channelId);
            // Broadcast to open channel room
            io.to(channelId).emit("receive_direct", { ...msg });
          }

          // Broadcast to recipient personal user rooms (pAId, pBId, toProviderId)
          const targets = new Set<string>();
          if (toProviderId) {
            targets.add(toProviderId.toString());
            const targetUser = await resolveChatUser(toProviderId.toString());
            if (targetUser?.id) targets.add(targetUser.id.toString());
          }

          if (channelId) {
            const channel = await prisma.chatChannel.findUnique({
              where: { id: channelId },
              select: { providerAId: true, providerBId: true },
            });
            if (channel) {
              targets.add(channel.providerAId.toString());
              targets.add(channel.providerBId.toString());
            }
          }

          const senderId = (msg.senderId || msg.sender?.id)?.toString();

          targets.forEach((targetId) => {
            if (targetId !== senderId) {
              io.to(targetId).emit("receive_direct", { ...msg });
              io.to(targetId).emit("refresh_unread", {
        ...
        try {
          ...
          if (!auth.callingAllowed) {
            socket.emit("call_error", {
              message:
                "Calling isn't available — one of the providers' trial calling access has ended.",
            });
            logCallEvent(
              appointmentId,
              auth.role,
          ...
          if (auth.role === "provider") {
            ...
            const { iceServers, turnCredentials } = getIceServers(
              appointmentId,
              auth.participantId,
            );
            if (!turnCredentials) {
              logger.warn(
                "TURN_SHARED_SECRET not set — falling back to STUN-only ICE servers for local/dev testing.",
              );
            }
            ...
            if (waitingSockets.length > 0) {
              ...
              const guestAuth = (
                guestSocket.data as { callParticipant?: CallAuthResult }
              ).callParticipant;
      ...
      async ({ appointmentId }: { appointmentId: string }) => {
        ...
        try {
          ...
          if (appointment && appointment.provider) {
            ...
            const callerName =
              appointment.bookingProvider?.user?.fullName ||
            ...
            io.to(`notification_room_${recipientUserId}`).emit(
              "new_notification",
              notification,
            );
          }
          ...
          logger.error(
            "[Socket] Failed to create missed call notification:",
            err,
          );
        }
      },
    ...
    socket.on(
      "webrtc_offer",
      ({ appointmentId, sdp }: { appointmentId: string; sdp: unknown }) => {
        const room = `call_${appointmentId}`;
    ...
    socket.on(
      "call_media_request",
      ({ appointmentId, kind }: { appointmentId: string; kind: string }) => {
        const room = `call_${appointmentId}`;
        if (!socket.rooms.has(room)) return;
    ...
    socket.on("disconnecting", () => {
      const participant = (socket.data as { callParticipant?: CallAuthResult })
        .callParticipant;
    ...
    socket.on("disconnect", () => {
      ...
      if (activeId) {
        ...
        userStatusMap.set(activeId, { isOnline: false, lastSeen });
        io.emit("user_status_changed", {
          userId: activeId,
          isOnline: false,
          lastSeen,
