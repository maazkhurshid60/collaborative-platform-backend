...
const generateClientId = async (): Promise<string> => {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    const hexPart = Math.floor(Math.random() * 0xffffff)
      .toString(16)
      .toUpperCase()
      .padStart(6, "0");
...
const getAllClients = asyncHandler(async (req: Request, res: Response) => {
  ...
  res
    ...
    .json(
      new ApiResponse(
        StatusCodes.OK,
        { totalDocument, clients: filteredClients },
...
export const getClientById = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!client) {
      return res
        .status(StatusCodes.NOT_FOUND)
        .json(new ApiResponse(StatusCodes.NOT_FOUND, null, "Client not found"));
    }
    ...
    if (loginRole === Role.superAdmin) {
      ...
      // @ts-ignore
      const isLinked = (client as any).providerList.some(
    ...
    // @ts-ignore
    const isBlocked =
      (loginUser as any).blockedMembers.includes(client.userId) ||
      (client.user as any).blockedMembers.includes(loginUserId);
    if (isBlocked) {
      return res
        .status(StatusCodes.FORBIDDEN)
        .json(
          new ApiResponse(
            StatusCodes.FORBIDDEN,
            null,
            "Cannot view profile due to block status.",
          ),
        );
    }
...
const deletClient = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!provider) {
    return res
      ...
      .json(
        new ApiResponse(
          StatusCodes.FORBIDDEN,
          { error: "Action allowed for providers only." },
          "",
        ),
      );
  }
  ...
  if (!link) {
    return res
      ...
      .json(
        new ApiResponse(
          StatusCodes.NOT_FOUND,
          { error: "Client is not linked to this provider." },
          "",
        ),
      );
  }
  ...
  await AuditLogService.createLog({
    ...
    resourceId: clientId,
    details: {
      message: "Provider removed client from their active list",
      providerId: provider.id,
    },
  });
  ...
  return res
    ...
    .json(
      new ApiResponse(
        StatusCodes.OK,
        null,
        "Client removed from your list successfully",
      ),
    );
});
...
const updateClient = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!isClientExist) {
    return res
      .status(StatusCodes.NOT_FOUND)
      .json(new ApiResponse(StatusCodes.NOT_FOUND, {}, "Client not found"));
  }
  ...
  const requestorProvider = await prisma.provider.findUnique({
    where: { userId: loginUserId },
  });

  if (
    loginUserRole !== Role.superAdmin &&
    (!requestorProvider ||
      (isClientExist as any).createdByProviderId !== requestorProvider.id)
  ) {
    return res
      .status(StatusCodes.FORBIDDEN)
      .json(
        new ApiResponse(
          StatusCodes.FORBIDDEN,
          {},
          "You do not have permission to edit this client. Only the original creator can modify this profile.",
        ),
      );
  }
  // -----------------------
...
const getTotalClient = asyncHandler(async (req: Request, res: Response) => {
  ...
  res
    ...
    .json(
      new ApiResponse(
        StatusCodes.OK,
        { totalDocument: allClient.length, clients: allClient },
        "All Clients fetched successfully",
      ),
    );
});
...
const addClient = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (req.body.isAccountCreatedByOwnClient)
    req.body.isAccountCreatedByOwnClient =
      req.body.isAccountCreatedByOwnClient === "true";
  ...
  if (!clientParsed.success) {
    return res
      ...
      .json(
        new ApiResponse(
          StatusCodes.BAD_REQUEST,
          { error: clientParsed.error.errors },
          "Validation failed",
        ),
      );
  }
    ...
    if (err?.code === "P2002") {
      return res
        ...
        .json(
          new ApiResponse(
            StatusCodes.CONFLICT,
            { error: "Duplicate value. Please use different credentials." },
            "Duplicate value.",
          ),
        );
    }
...
const addExistingClientToProvider = asyncHandler(
  async (req: Request, res: Response) => {
    if (req.body.age) req.body.age = Number(req.body.age);
    if (req.body.isAccountCreatedByOwnClient)
      req.body.isAccountCreatedByOwnClient =
        ...
        return res
          ...
          .json(
            new ApiResponse(
              StatusCodes.BAD_REQUEST,
              null,
              "This email is registered but not as a client",
            ),
          );
      }
        ...
        return res
          ...
          .json(
            new ApiResponse(
              StatusCodes.NOT_FOUND,
              null,
              "Client record not found for existing license number",
            ),
          );
      }
        ...
        return res
          ...
          .json(
            new ApiResponse(
              StatusCodes.CONFLICT,
              null,
              "This provider has already added the client",
            ),
          );
      }
        ...
        return res
          ...
          .json(
            new ApiResponse(
              StatusCodes.CREATED,
              clientCreated,
              "Client Data has been already sent to the super admin for verification. Client will receive a verification email once approved, after which Client will be able to log in.",
            ),
          );
      }
      return res
        ...
        .json(
          new ApiResponse(
            StatusCodes.CREATED,
            existingClient,
            "Existing client linked to provider successfully",
          ),
        );
    }
    ...
    if (role === Role.client) {
      ...
      try {
        ...
        console.error(
          "[addExistingClientToProvider] Welcome email failed (non-fatal):",
          emailErr,
        );
      }
      ...
      return res
        ...
        .json(
          new ApiResponse(
            StatusCodes.CREATED,
            clientCreated,
            "Client created and linked to provider successfully. A welcome email with their Client ID has been sent.",
          ),
        );
    }
...
const updateExistingClientOnLicenseNo = asyncHandler(
  async (req: Request, res: Response) => {
    const clientData = clientSchema.safeParse(req.body);
    if (!clientData.success) {
      return res
    ...
    if (email !== isClientExist.user.email?.trim().toLowerCase()) {
      // Check user email
      ...
      if (isEmailExist) {
        return res
          ...
          .json(
            new ApiResponse(
              StatusCodes.CONFLICT,
              { error: `Email ${email} already taken by another client` },
              "Duplicate Email",
            ),
          );
      }
