...
const getAllDocumentApi = asyncHandler(async (req: Request, res: Response) => {
  ...
  const completedDocuments = allDocuments.filter((doc) =>
    agreedDocs.includes(doc.id),
  );

  const uncompletedDocuments = allDocuments.filter(
    (doc) => !agreedDocs.includes(doc.id) && !onlySharedDocs.includes(doc.id),
  );
  const sharedDocuments = allDocuments.filter((doc) =>
    onlySharedDocs.includes(doc.id),
  );
...
const documentSharedWithClientApi = asyncHandler(
  async (req: Request, res: Response) => {
    const { providerId, clientId, documentId, senderId, clientEmail } =
      req.body;
    logger.debug(
      `Document share request: providerId=${providerId}, clientId=${clientId}, senderId=${senderId}, documents=${documentId}`,
    );
    ...
    for (const doc of sharedDocuments) {
      ...
      if (!clientUser || !providerUser) {
        logger.warn(
          `User not found for client or provider in document sharing.`,
        );
        continue;
      }
      ...
      io.to(`notification_room_${providerUser.id}`).emit(
        "new_notification",
...
const documentSignByClientApi = asyncHandler(
  async (req: Request, res: Response) => {
    const { clientId, sharedDocumentId, isAgree, senderId, eSignature } =
      req.body;
    logger.debug(
      `Document sign request: clientId=${clientId}, sharedDocumentId=${sharedDocumentId}, isAgree=${isAgree}`,
    );
    ...
    if (!isShareDocumentExist) {
      return res
        ...
        .json(
          new ApiResponse(
            StatusCodes.NOT_FOUND,
            { error: "Document not found or not assigned to this client." },
            "Not Found",
          ),
        );
    }
    ...
    if (!providerUserId || !clientUserId) {
      return res
        .status(StatusCodes.NOT_FOUND)
        .json(new ApiResponse(StatusCodes.NOT_FOUND, {}, "User IDs missing."));
    }
    ...
    io.to(`notification_room_${clientUserId}`).emit(
      "new_notification",
      clientNotification,
    );
...
const getAllSharedDocumentWithClientApi = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const getAllSharedDocument = await prisma.documentShareWith.findMany({
      where: { clientId },
      include: { document: true, provider: { include: { user: true } } },
    });
 ...
 */
const getDocumentRecipientsApi = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    const providerId =
 ...
 */
const getAllMasterDocumentsApi = asyncHandler(
  async (req: Request, res: Response) => {
    const providerId =
      (req.query.providerId as string) ||
      (req.body?.providerId as string) ||
    ...
    const documents = await prisma.document.findMany({
      include: {
        sharedWith: providerId ? { where: { providerId } } : true,
      },
      orderBy: { createdAt: "desc" },
    });
...
export {
  addDocumentApi,
  getAllDocumentApi,
  documentSharedWithClientApi,
  documentSignByClientApi,
  getAllSharedDocumentWithClientApi,
  deleteDocumentApi,
  getAllMasterDocumentsApi,
  getDocumentRecipientsApi,
};
