...
const createGroupApi = asyncHandler(async (req: Request, res: Response) => {
  ...
  const isDuplicateGroupName = await prisma.groupChat.findFirst({
  ...
  if (membersRoleCheck.length !== membersId.length) {
    return res
      ...
      .json(
        new ApiResponse(
          StatusCodes.CONFLICT,
          { message: `Only providers can be added to groups.` },
          "Duplicate Error.",
        ),
      );
  }
...
const updateGroupApi = asyncHandler(async (req: Request, res: Response) => {
  ...
  const member = await prisma.provider.findFirst({
  ...
  if (!member) {
    return res.status(StatusCodes.NOT_FOUND).json(
      new ApiResponse(
        ...
        {
          message: `Member not found with email: ${memberEmail}. Please create an account. Once verified, you can join the group using this link.`,
        },
        "Member Error.",
      ),
    );
  ...
  const isMemberAlreadyInGroup = currentMembers.members.some(
    ...
    return res
      ...
      .json(
        new ApiResponse(
          StatusCodes.CREATED,
          { updateGroup },
          "You have joined the group. Please log in yourself and go to the chat for more information.",
        ),
      );
  }
 ...
 */
const addExistingProvidersToGroupApi = asyncHandler(
  async (req: Request, res: Response) => {
    const { groupId, providerIds } = req.body as {
      groupId?: string;
      providerIds?: string[];
    };
      ...
      return res
        ...
        .json(
          new ApiResponse(
            ...
            null,
            "groupId and providerIds[] are required",
          ),
        );
    }
    ...
    if (!group) {
      return res
        ...
        .json(new ApiResponse(StatusCodes.NOT_FOUND, null, "Group not found"));
    }

    // 2. Caller must be a member of the group.
    if (
      !callerUserId ||
      !group.members.some((m) => m.userId === callerUserId)
    ) {
      return res
        ...
        .json(
          new ApiResponse(
            ...
            null,
            "Only group members can add others to this group",
          ),
        );
    }
      ...
      return res
        ...
        .json(
          new ApiResponse(
            StatusCodes.BAD_REQUEST,
            null,
            "One or more providers are invalid",
          ),
        );
    }
      ...
      return res
        ...
        .json(
          new ApiResponse(
            StatusCodes.CONFLICT,
            null,
            "All selected providers are already in this group",
          ),
        );
    }
    ...
    await AuditLogService.createLog({
      ...
      resourceId: groupId,
      details: {
        addedMemberCount: toAdd.length,
        addedMemberNames: toAdd.map((p) => p.user?.fullName),
      },
    });
 ...
 */
const updateGroupPermissionsApi = asyncHandler(
  async (req: Request, res: Response) => {
    const { groupId, membersCanInvite } = req.body as {
      groupId?: string;
      ...
      return res
        ...
        .json(
          new ApiResponse(
            StatusCodes.BAD_REQUEST,
            null,
            "groupId and membersCanInvite (boolean) are required",
          ),
        );
    }
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const provider = await prisma.provider.findFirst({
        where: {
          OR: [{ id: senderId }, { userId: senderId }],
        },
        select: { userId: true },
      });
      ...
      // Handle email notifications with 20-minute cooldown
      const COOLDOWN_MINUTES = 20;
      const now = new Date();
      console.log(
        `[Email Debug] Checking members to notify for group ${groupId}. Total members:`,
        groupMembers.members.length,
      );

      const membersToNotify = groupMembers.members.filter((member) => {
        if (member.userId === userIdToUse) {
          console.log(`[Email Debug] Skipping sender (${userIdToUse})`);
          return false;
        }

        // If no email sent yet, or the last email was sent more than 20 minutes ago
        if (!member.lastEmailSentAt) {
          console.log(
            `[Email Debug] Will notify member ${member.userId} (No previous email sent)`,
          );
          return true;
        }

        const diffMs =
          now.getTime() - new Date(member.lastEmailSentAt).getTime();
        const diffMins = diffMs / (1000 * 60);

        if (diffMins >= COOLDOWN_MINUTES) {
          console.log(
            `[Email Debug] Will notify member ${member.userId} (Last email ${diffMins.toFixed(2)} mins ago)`,
          );
          return true;
        } else {
          console.log(
            `[Email Debug] Skipping member ${member.userId} (Last email only ${diffMins.toFixed(2)} mins ago, within cooldown)`,
          );
          return false;
        }
      });
      ...
      console.log(
        `[Email Debug] Found ${membersToNotify.length} members to notify`,
      );

      if (membersToNotify.length > 0) {
        if (emailQueue) {
          const emailJobs = membersToNotify.map((member) => ({
...
const getGroupMessageApi = asyncHandler(async (req: Request, res: Response) => {
  ...
  const unreadMessagesCount = groupMessagesWithReadStatus.filter(
...
const getPublicGroupMessageApi = asyncHandler(
  async (req: Request, res: Response) => {
...
const getAllGroupsApi = asyncHandler(async (req: Request, res: Response) => {
  ...
  const provider = await prisma.provider.findFirst({
    where: {
      OR: [{ id: loginUserId }, { userId: loginUserId }],
    },
    select: { userId: true },
  });
...
const shareGroupChatByEmail = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!group) {
        return res
          .status(StatusCodes.NOT_FOUND)
          .json({ message: "Group chat not found" });
      }
          ...
          return res
            ...
            .json(
              new ApiResponse(
                StatusCodes.CONFLICT,
                null,
                "User is already a member of this group chat.",
              ),
            );
        }
          ...
          return res
            ...
            .json(
              new ApiResponse(
                StatusCodes.INTERNAL_SERVER_ERROR,
                null,
                "Cannot create invitation: no provider found to act as inviter.",
              ),
            );
        }
      ...
      return res
        ...
        .json(
          new ApiResponse(
            StatusCodes.OK,
            null,
            "Invitation email sent successfully. They will join the group upon signing up.",
          ),
        );
    } catch (error) {
      ...
      return res
        .status(500)
        .json({ message: "Error sharing group chat", error });
    }
  },
);
