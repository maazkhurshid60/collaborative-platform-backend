...
const getAllUnblockProviders = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const filteredProviders = allProviders.filter(
      (provider) =>
        !loginUser.blockedMembers.includes(provider.user.id) &&
        provider.user.id !== loginUserId,
    );
    ...
    res
      ...
      .json(
        new ApiResponse(
          StatusCodes.OK,
          { totalDocument: totalDocument, providers: filteredProviders },
...
const getTotalProviders = asyncHandler(async (req: Request, res: Response) => {
  ...
  res
    ...
    .json(
      new ApiResponse(
        StatusCodes.OK,
        { totalDocument: allProviders.length, providers: allProviders },
        "All Providers fetched successfully",
      ),
    );
});
...
const deletProvider = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error(
      "Failed to safely cancel stripe sub during account delete",
      error,
    );
  }

  const isProviderDeleted = await prisma.provider.delete({
    where: { id: providerId },
  });
  ...
  return res
    ...
    .json(
      new ApiResponse(
        StatusCodes.OK,
        { isProviderDeleted },
        `${isProviderExist.user.email} deleted successfully`,
      ),
    );
});
...
const updateProvider = asyncHandler(async (req: Request, res: Response) => {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          ...
          { error: providerData.error.errors },
          "Validation Failed",
        ),
      );
  }
  ...
  if (isLicenseNoExists) {
    return res
      ...
      .json(
        new ApiResponse(
          StatusCodes.CONFLICT,
          { error: `License Number ${licenseNo} already taken` },
          "Duplicate Error",
        ),
      );
  }
  ...
  const isFullNameExist = await prisma.user.findFirst({
    where: { fullName, id: { not: isProviderExist.userId } },
  });
  if (isFullNameExist) {
    return res
      .status(StatusCodes.CONFLICT)
      .json(
        new ApiResponse(
          StatusCodes.CONFLICT,
          { error: `Full Name ${fullName} already taken` },
          "Duplicate Error",
        ),
      );
  }
