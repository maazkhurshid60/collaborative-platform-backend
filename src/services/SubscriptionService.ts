...
export class SubscriptionService {
  private async syncUserToKit(userId: string) {
    try {
      ...
      if (
        ...
        await kitQueue.add("sync-subscriber", {
          email: user.email,
          fullName: user.fullName,
        });
        logger.info(
          `Enqueued Kit sync for updated subscriber ${user.email} (userId: ${userId})`,
        );
      }
    } catch (err: any) {
      logger.error(
        `Failed to trigger Kit sync for user ${userId}: ${err.message}`,
      );
    }
  }
  ...
  private resolveBillingCycle(stripeSub: any, fallbackPeriod?: string): string {
    ...
    const meta =
      stripeSub?.metadata?.period || stripeSub?.metadata?.billingCycle;
  ...
  private resolvePlanType(stripeSub: any, fallbackPlan?: string): string {
    const priceId =
      stripeSub?.items?.data?.[0]?.price?.id || stripeSub?.plan?.id;

    if (
      priceId === STRIPE_PRICES.STANDARD.MONTHLY ||
      priceId === STRIPE_PRICES.STANDARD.YEARLY
    ) {
      return "STANDARD";
    }
  ...
  async createCheckoutSession(
    ...
    if (!priceId)
      throw new ApiError(StatusCodes.BAD_REQUEST, "Invalid plan or period");
  ...
  async cancelSubscription(userId: string, reason?: string) {
    ...
    this.syncUserToKit(userId).catch(() => {});
    return { success: true };
  }
  ...
  async createSubscriptionIntent(data: {
    ...
    if (!customerId) {
      try {
        const existingCustomers = await stripeService.listCustomers({
          email,
          limit: 1,
        });
        ...
        logger.warn(`Could not search for existing customer: ${e.message}`);
      }
    }
    ...
    if (!customerId) {
      ...
      try {
        ...
        logger.warn(
          `Could not update existing Stripe customer name: ${e.message}`,
        );
      }
    }
    ...
    let subscription = existingStripeSubs.data.find(
      (s: any) =>
        s.status === "incomplete" && s.items.data[0].price.id === priceId,
    );

    // Cancel other incomplete/trialing subscriptions to prevent duplicate "incomplete" rows in Stripe
    const othersToCancel = existingStripeSubs.data.filter(
      (s: any) =>
    ...
    for (const subToCancel of othersToCancel) {
      try {
        ...
        logger.warn(
          `Could not cancel orphaned incomplete sub ${subToCancel.id}: ${e.message}`,
        );
      }
    }
    ...
    if (user && !subscription) {
      ...
      if (
        ...
        try {
          ...
          logger.warn(`Could not cancel old sub in DB: ${e.message}`);
        }
      }
    }
    ...
    if (!subscription) {
      ...
      if (
        isUpgrade &&
        (subscription.status === "trialing" || (subscription as any).trial_end)
      ) {
    ...
    if (!clientSecret && latestInvoice?.id) {
      const freshInvoice = await stripeService.retrieveInvoice(
        latestInvoice.id,
      );
      clientSecret =
        (freshInvoice as any).confirmation_secret?.client_secret ||
    ...
    if (!clientSecret) {
      throw new ApiError(
        ...
        "Could not retrieve payment client secret. Please try again.",
      );
    }
  ...
  private formatPayment(payment: any) {
    let planName =
      payment.plan || payment.user.subscription?.plan || "Standard";
    ...
    const invoiceNo =
      ...
      `INV-${new Date(payment.createdAt).getFullYear()}-${payment.id.split("-")[0].toUpperCase()}`;
    ...
    return {
      id: payment.id,
      invoiceNo,
      date: (payment.periodStart || payment.createdAt)
        .toISOString()
        .split("T")[0],
  ...
  async syncSubscription(userId: string) {
    ...
    if (sub && !sub.stripeSubscriptionId) {
      ...
      const customerId =
        user?.stripeCustomerId || (sub as any).stripeCustomerId;
      if (customerId) {
        try {
          const stripeSubs = await stripeService.listSubscriptions({
            customer: customerId,
            limit: 1,
            status: "active",
          });
          if (stripeSubs?.data?.length > 0) {
            ...
            sub = await prisma.subscription.update({
              ...
              data: {
                ...
                stripeCustomerId: customerId,
                ...(foundEndUnix && {
                  currentPeriodEnd: new Date(foundEndUnix * 1000),
                }),
                status: "ACTIVE",
              },
            });
          ...
          /* could not find from Stripe */
        }
      }
    }

    if (!sub || !sub.stripeSubscriptionId)
      throw new ApiError(StatusCodes.NOT_FOUND, "No stripe subscription found");
    ...
    try {
      const latestInvoices = await stripeService.listInvoices({
      ...
      /* fallback to subscription current_period_end below */
    }
    ...
    const derivedStatus = confirmedPeriodEnd
      ? "ACTIVE"
      : stripeSub.status === "trialing" && sub.status === "ACTIVE"
        ? "ACTIVE"
        : statusMap[stripeSub.status] || "ACTIVE";

    const stripeSubEndUnix = this.getSubPeriodEnd(stripeSub);
    const fallbackPeriodEnd = stripeSubEndUnix
      ? new Date(stripeSubEndUnix * 1000)
    ...
    const resolveCustomerLast4 = async (
      customerId: string,
      subscriptionId: string,
    ): Promise<string | null> => {
      try {
        const subWithPm: any =
          await stripeService.retrieveSubscriptionExpanded(subscriptionId);
        const last4 = subWithPm?.default_payment_method?.card?.last4 ?? null;
        if (last4) {
          console.log(`[sync] last4 from sub.default_payment_method: ${last4}`);
          return last4;
        }
      } catch (e) {
        /* fall through */
      ...
      try {
        ...
        if (last4) {
          console.log(`[sync] last4 from customer PM list: ${last4}`);
          return last4;
        }
      } catch (e) {
        /* fall through */
    ...
    const resolveInvoiceLast4 = async (
      invoice: any,
      customerId: string,
      subscriptionId: string,
    ): Promise<string | null> => {
      // 1. Customer/subscription level (most reliable in new API)
      const customerLast4 = await resolveCustomerLast4(
        customerId,
        subscriptionId,
      );
      ...
      if (!piId && (invoice as any).confirmation_secret?.client_secret) {
        piId =
          (invoice as any).confirmation_secret.client_secret.split(
            "_secret_",
          )?.[0] ?? null;
      }
      if (piId?.startsWith("pi_")) {
        try {
          const pi: any = await stripeService.retrievePaymentIntent(piId, [
            "latest_charge",
            "payment_method",
          ]);
          const l4 =
    ...
    for (const invoice of invoices.data) {
      if (invoice.status === "paid" && invoice.amount_paid >= 0) {
        ...
        if (!existing) {
          ...
          try {
            last4 = await resolveInvoiceLast4(
              invoice,
              stripeCustomerId ?? sub.stripeCustomerId ?? "",
              sub.stripeSubscriptionId ?? "",
            );
            console.log(
              `[sync] new payment last4: ${last4} for invoice ${invoice.id}`,
            );
          } catch (e) {
            ...
            await prisma.payment.create({
              data: {
                ...
                stripePaymentIntentId:
                  (invoice.payment_intent as string) || "synced_invoice",
          ...
          try {
            const last4 = await resolveInvoiceLast4(
              invoice,
              stripeCustomerId ?? sub.stripeCustomerId ?? "",
              sub.stripeSubscriptionId ?? "",
            );
            if (last4) {
              ...
              console.log(
                `[sync] patched last4 ${last4} on payment ${existing.id}`,
              );
            } else {
              console.log(
                `[sync] could not resolve last4 for existing payment ${existing.id}`,
              );
            }
          } catch (e) {
  ...
  private async handleCheckoutSessionCompleted(session: any) {
    ...
    if (targetUserId) {
      ...
      this.syncUserToKit(targetUserId).catch(() => {});

      // Cleanup payment methods: set new as default and remove old ones
      if (session.payment_intent) {
        try {
          const pi = await stripeService.retrievePaymentIntent(
        ...
        try {
          const si = await stripeService.retrieveSetupIntent(
      ...
      try {
        ...
        if (fullUser) {
          ...
          const nextBillingDateStr = currentPeriodEnd
        ...
        logger.error(
          `Failed to send subscription success email: ${emailErr.message}`,
        );
      }
    }
  }
  ...
  private async handleInvoicePaymentSucceeded(invoice: any) {
    ...
    if (!subscriptionId) {
      logger.warn(
        `[invoice.payment_succeeded] No subscription ID resolvable on invoice ${invoice?.id}. Skipping.`,
      );
      return;
    }
    ...
    if (!subscription) {
      ...
      if (targetUserId) {
        ...
        const subPeriodEnd = subPeriodEndUnix
          ? new Date(subPeriodEndUnix * 1000)
          : undefined;
    ...
    if (subscription) {
      ...
      if (invoice.amount_paid >= 0) {
        ...
        if (paymentRef && String(paymentRef).startsWith("pi_")) {
          try {
            ...
            logger.debug(
              `Expanded PaymentIntent ID: ${pi.id}, Status: ${pi.status}, Charge ID: ${charge?.id}, Card: ${charge?.payment_method_details?.card?.brand} **** ${charge?.payment_method_details?.card?.last4}`,
            );
          } catch (e: any) {}
        }
        ...
        if (expandedLast4) {
          try {
            ...
            if (newPayment && !newPayment.paymentMethodLast4) {
              ...
              console.log(
                `✅ Patched new payment ${newPayment.id} with last4: ${expandedLast4}`,
              );
            }
          } catch (e) {
        ...
        if (pmId && typeof pmId === "string") {
          ...
          try {
            const pi = await stripeService.retrievePaymentIntent(paymentRef, [
              "payment_method",
            ]);
            const pmIdFromPi =
              typeof pi.payment_method === "string"
            ...
            logger.error(
              `Error retrieving PM from PI for cleanup: ${e.message}`,
            );
          }
        }
      }
      if (invoice.billing_reason === "subscription_cycle" || subscriptionId) {
        ...
        const nextBillingUnix =
          ...
          this.getSubPeriodEnd(stripeSub);
        ...
        io.to(`notification_room_${subscription.userId}`).emit(
      ...
      if (invoice.billing_reason === "subscription_create") {
        try {
          ...
          if (fullUser) {
            ...
            const billingCycle =
              stripeSub.metadata?.period ||
              (stripeSub.plan?.interval === "year" ? "YEARLY" : "MONTHLY");
          ...
          logger.error(
            `Failed to send subscription success email from invoice handler: ${emailErr.message}`,
          );
        }
      }
      ...
      this.syncUserToKit(subscription.userId).catch(() => {});
    }
  }
  ...
  private async handleInvoicePaymentFailed(invoice: any) {
    ...
    if (!subscriptionId) {
      logger.warn(
        `[invoice.payment_failed] No subscription ID resolvable on invoice ${invoice?.id}. Skipping.`,
      );
      return;
    }
    ...
    if (sub) {
      await prisma.payment.create({
        data: {
          ...
          plan: sub.plan,
          stripePaymentIntentId:
            (invoice.payment_intent as string) || "failed_invoice",
          stripeInvoiceId: invoice.id,
          invoiceUrl: invoice.hosted_invoice_url,
        },
      });
  ...
  private async handleSubscriptionDeleted(subscription: any) {
    ...
    if (deletedSub) {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const existingRecord = await tx.payment.findFirst({
          where: { stripeInvoiceId: invoiceId },
        });
        if (!existingRecord) {
          await tx.payment.create({
      ...
      io.to(`notification_room_${deletedSub.userId}`).emit(
        "subscription_updated",
      );
      this.syncUserToKit(deletedSub.userId).catch(() => {});

      // Send custom Cancellation Email
      try {
        const fullUser = await prisma.user.findUnique({
          where: { id: deletedSub.userId },
        });
        if (fullUser) {
        ...
        logger.error(
          `Failed to send subscription cancellation email: ${emailErr.message}`,
        );
      }
    }
  }
    ...
    if (existingSub) {
      ...
      await prisma.subscription.update({
        ...
        data: {
          ...
          billingCycle: this.resolveBillingCycle(
            subscription,
            existingSub.billingCycle ?? undefined,
          ),
      ...
      io.to(`notification_room_${existingSub.userId}`).emit(
        "subscription_updated",
      );
      this.syncUserToKit(existingSub.userId).catch(() => {});
    }
  }
  ...
  private async recordPayment(
    ...
    try {
      ...
      if (
        (!piId || piId === "webhook") &&
        invoiceData?.confirmation_secret?.client_secret
      ) {
        piId =
          invoiceData.confirmation_secret.client_secret.split("_secret_")?.[0];
      }
    ...
    if (!last4 && invoiceData?.charge) {
      try {
        const charge = await stripeService.retrieveCharge(
          invoiceData.charge as string,
        );
  ...
  private async handleChargeSucceeded(charge: any) {
    ...
    if (invoiceId) {
      ...
      try {
        ...
        logger.info(
          `✅ Upserted partial payment for invoice ${invoiceId} with last4: ${last4}`,
        );
      } catch (error: any) {
        // unique contrint race on concurrent webhooks - safe ingore
        if (error?.code !== "P2002")
          logger.error(
            `Error upserting payment in charge.succeeded: ${error.message}`,
          );
      }
    } else {
      ...
      if (recent && !recent.paymentMethodLast4) {
        ...
        console.log(
          `✅ Patched recent payment ${recent.id} with last4: ${last4}`,
        );
      }
    }
  ...
  private async handlePaymentMethodAttached(paymentMethod: any) {
    ...
    logger.info(
      `[StripeWebhook] Payment method ${pmId} attached to customer ${customerId}`,
    );
    ...
    try {
      ...
      if (user) {
        ...
        if (latestPayment) {
          ...
          logger.info(
            `Updated payment ${latestPayment.id} with last4: ${last4}`,
          );
        }
  ...
  private async cleanupPaymentMethods(
    customerId: string,
    newPaymentMethodId: string,
  ) {
    logger.info(
      `[StripeCleanup] Starting cleanup for customer ${customerId}. New PM: ${newPaymentMethodId}`,
    );
    try {
      ...
      logger.info(
        `[StripeCleanup] Set ${newPaymentMethodId} as default for ${customerId}`,
      );
      ...
      for (const pm of paymentMethods.data) {
        if (pm.id !== newPaymentMethodId) {
          try {
            ...
            logger.info(
              `[StripeCleanup] Successfully detached old PM: ${pm.id}`,
            );
            detachedCount++;
          } catch (detachError: any) {
            logger.error(
              `[StripeCleanup] Failed to detach PM ${pm.id}: ${detachError.message}`,
            );
          }
        }
      }
      logger.info(
        `[StripeCleanup] Cleanup complete for ${customerId}. Detached ${detachedCount} old methods.`,
      );
    } catch (error: any) {
      logger.error(
        `[StripeCleanup] CRITICAL Error during cleanup for ${customerId}: ${error.message}`,
      );
    }
  }
  ...
  public async cancelStripeSubscription(userId: string) {
    try {
      ...
      logger.error(
        `Error canceling Stripe subscription for user: ${userId} ${error.message}`,
      );
