...
export class UserService {
  ...
  async updateMe(loginUserId: string, updateData: any) {
    ...
    else if (gender === "prefer_not_to_say" || gender === "PREFER_NOT_TO_SAY")
  ...
  async deleteMe(userId: string) {
    ...
    try {
      ...
      console.error(
        "Failed to safely cancel stripe sub during deleteMe",
        error,
      );
    }
  ...
  async deleteUser(userId: string) {
    ...
    if (user.stripeCustomerId) {
      try {
        ...
        console.error(
          `Failed to safely cancel stripe sub for user ${userId} during deletion`,
          error,
        );
      }
    }
